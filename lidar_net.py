...
BATCH_SIZE = 1
MODEL_PATH = "lidar_building_model.keras"
...
def weighted_loss(y_true, y_pred):
    # y_pred here are logits, as required by weighted_cross_entropy_with_logits
    return weighted_cross_entropy_with_logits(labels=y_true, logits=y_pred, pos_weight=50)
...
# Challenge question: Save model

model.save(MODEL_PATH)
print(f"\nSaved trained model to {MODEL_PATH}")


# Challenge question: Reload model
...
# Optional: evaluate reloaded model on validation set
print("\nEvaluating reloaded model on validation set...")
valid_eval_gen = BatchGenerator(valid_tiles, '/home/drake/lidar/lidar_chm', '/home/drake/lidar/lidar_tag', batch_size=BATCH_SIZE)
reloaded_model.evaluate(valid_eval_gen, verbose=2)

# Per-batch predicted vs actual building pixels (using reloaded model)
...
for chm, tag in valid_gen:
    if chm.size == 0:  # For some reason we get an empty image for the last batch
        break
    # Note: BatchGenerator returns shape (N, 1000, 1000); model expects (N, 1000, 1000, 1)
    # If needed, we can expand dims here, but if your environment already works, this is fine.
    predict_logits = reloaded_model.predict(chm, verbose=0)
    predict = predict_logits > 0.5
    print(f'\nBatch {i}: {np.sum(predict)} predicted, {np.sum(tag)} actual')
    i += 1

# Uncomment this if you are done tweaking parameters and want a test set evaluation as well:
# print('\nTESTING')
# test_gen = BatchGenerator(test_tiles, '/home/drake/lidar/lidar_chm', '/home/drake/lidar/lidar_tag', batch_size=BATCH_SIZE)
# reloaded_model.evaluate(test_gen, verbose=2)